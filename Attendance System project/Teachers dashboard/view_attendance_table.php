...
<body>
    <form action="../database and tables/attendace_record_insert.php"
        ...
        try {
            ...
            where Student_table.semester = '$semester_id' 
            AND Student_table.Name like '%$searched%' OR Student_table.RollNO like '%$searched%' OR Year_table.Name like '%$searched%' OR Semester_table.Name like '%$searched%' OR batch_table.Title like '%$searched%' AND
             Student_table.status = '1' ORDER BY Student_table.RollNO ASC";
            } else {
                $student_select = "SELECT Student_table.*, batch_table.Title as batch_name, course_table.Name as course_name , year_table.Name as year_name, Semester_table.Name as semester
                 FROM Student_table 
                INNER JOIN batch_table ON Student_table.batch_id = batch_table.id
                ...
                INNER JOIN year_table ON Student_table.year = year_table.id
                INNER JOIN semester_table ON Student_table.semester = semester_table.id
                where Student_table.semester = '$semester_id' AND
                 Student_table.status = '1' ORDER BY Student_table.RollNO ASC";
            }
            ...
            if (mysqli_num_rows($student_result) > 0) {
                ...
                echo "<center><h2>subject name : " .$sub_name."</h2></center>";
                echo '<table border="1">
                <tr>
                    <th>profile</th>          
            ...
            <tr>
                ...
                <button  class="save_attendance" name="save_sttendance">Save attendance</button>  
                </td>
            </tr>
        ';
                echo '</table>';
    ...
    <script>
        // to change the background color of row while teacher click on present/absent checkbox
