...
$teacher_name = $teacher_email = $teacher_phone = $teacher_address = $teacher_pwd = $teacher_confirm_pwd = $teacher_uname = '';
$status = '1';
$teacher_error = 0;
if (isset($_POST['teacher_add_btn'])) {
    ...
    if (isset($_POST['teacher_address']) && !empty($_POST['teacher_address']) && trim($_POST['teacher_address'])) {
        ...
        $teacher_error++;
        $teacher_address_err= 'Enter your address';
    }
        ...
        if ($_FILES['ppimage']['error'] == 0) {
            if ($_FILES["ppimage"]["size"] < 5000000) { // Limiting to 50MB
                ...
                if (in_array($_FILES['ppimage']['type'], $filetype)) {
                    ...
                    if (move_uploaded_file($_FILES['ppimage']['tmp_name'], 'teacher_images/' . $filename)) {
                        $image = $filename;
            ...
            $teacher_error++;
            $teacher_img_err = "File upload error";
        }
    }
    ...
    //teacher passwword
    if (isset($_POST['teacher_pwd']) && !empty($_POST['teacher_pwd']) && trim($_POST['teacher_pwd'])) {
        $teacher_pwd = $_POST['teacher_pwd'];
        if (!preg_match('/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,}$/', $teacher_pwd)) {
            $teacher_error++;
            $teacher_pwd_err = 'your pwd is not valid';
        }else{
            $teacher_pwd = $_POST['teacher_pwd'];
        }
    } else {
        $teacher_error++;
        $teacher_pwd_err = 'Enter your password';
    }
    //teacher confirm password
    if (isset($_POST['teacher_confirm_pwd']) && !empty($_POST['teacher_confirm_pwd']) && trim($_POST['teacher_confirm_pwd'])) {
        $teacher_confirm_pwd = $_POST['teacher_confirm_pwd'];
        if ($teacher_confirm_pwd == $teacher_pwd) {
            $teacher_confirm_pwd = $_POST['teacher_confirm_pwd'];
        } else {
            $teacher_error++;
            $teacher_confirm_err = "confirm password is not matched";
        }
    } else {
        $teacher_error++;
        $teacher_confirm_err = 'Enter your confirm password';
    }
    if ($teacher_error == 0) {
        ...
        include '../database and tables/create_database.php';
        include '../database and tables/Teacher_table.php';
    } else {
    ...
    <form action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data">
        <div class="teacher_registration">
            ...
            <div>
                ...
                <input type="text" name="teacher_name" id="teacher_name" value="<?php echo $teacher_name; ?>" />
            ...
            <div>
                ...
                <input type="email" name="teacher_email" class="teacher_email" id="teacher_email"
                    placeholder="enter email" value="<?php echo $teacher_email; ?>" />
            ...
            <div>
                ...
                <input type="number" name="teacher_phone" class="tecaher_phone" id="teacher_phone"
                    placeholder="Enter contacts number" value="<?php echo $teacher_phone; ?>" />
            ...
            <div>
                ...
                <input type="text" name="teacher_address" class="teacher_address" id="teacher_address"
                    placeholder="Enter temporary" value="<?php echo $teacher_address; ?>" />
            ...
            <div>
                ...
                <div class="teacher_gender_div">
                    Male<input type="radio" class="teacher_gender" name="teacher_gender" value="Male <?php if ($teacher_gender == "Male")
                        echo "checked"; ?>" />
                    Female<input type="radio" class="teacher_gender" name="teacher_gender" value="Female <?php if ($teacher_gender == "Female")
                        echo "checked"; ?>" />
                    Others<input type="radio" class="teacher_gender" name="teacher_gender" value="others <?php if ($teacher_gender == "others")
                        echo "checked"; ?>" />
            ...
            <div>
                <div>
                    ...
                    <input type="text" name="teacher_uname" class="teacher_uname" id="teacher_uname"
                        placeholder="set a username  for login" value="<?php echo $teacher_uname; ?>" />
                    <span><?php echo isset($teacher_uname_err) ? $teacher_uname_err : ' '; ?></span>
                </div>
                <div>
                    <label for="teacher_pwd"> Password:</label>
                    <input type="password" name="teacher_pwd" class="teacher_pwd" id="teacher_pwd"
                        placeholder="set a passwrod for login" value="<?php echo $teacher_pwd; ?>" />
                    <span><?php echo isset($teacher_pwd_err) ? $teacher_pwd_err : ' '; ?></span>
                </div>
                <div>
                    <label for="teacher_confirm_pwd">Conform:</label>
                    <input type="password" name="teacher_confirm_pwd" class="teacher_confirm_pwd"
                        id="teacher_confirm_pwd" placeholder="conform your login password"
                        value="<?php echo $teacher_confirm_pwd; ?>" />
                    <span> <?php echo isset($teacher_confirm_err) ? $teacher_confirm_err : ' '; ?></span>
                </div>
            </div>
            <div>
